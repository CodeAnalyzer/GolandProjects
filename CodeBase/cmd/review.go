...
func parseReviewRules(raw string) ([]review.RuleID, []string, error) {
	...
	for _, part := range parts {
		...
		switch rule {
		case review.RuleForeignTablesUsing, review.RuleForeignPTablesUsing, review.RuleForeignProcedureUsing, review.RuleExecNotExistsProc, review.RuleProcDuplicate, review.RuleProcParamDefValue, review.RuleProcElseCase, review.RuleUseSelectAll, review.RuleTruncTbl, review.RuleDatatype, review.RuleAnsiInJoin, review.RuleInsertRowLock:
		default:
...
func init() {
	...
	reviewCmd.Flags().StringVar(&reviewRulesRaw, "rules", "", "comma-separated rules (foreignTablesUsing,foreignPTablesUsing,foreignProcedureUsing,execNotExistsProc,procDuplicate,procParamDefValue,procElseCase,useSelectAll,truncTbl,datatype,ansiInJoin,insertRowLock)")
