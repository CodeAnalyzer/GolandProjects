...
Система SHALL применять per-tool таймаут через `context.WithTimeout` поверх контекста запроса. По умолчанию для query/RTI/TRC инструментов используется `query_timeout_sec` (30с), для `codebase_review_sql` — `review_timeout_sec` (120с). Значения читаются из секции `[mcp]` конфигурации `codebase.toml`. При `timeout = 0` таймаут не применяется (вызов выполняется без ограничения). При истечении таймаута инструмент возвращает structured JSON-ошибку контекста отмены.
...
- **GIVEN** конфигурация с `query_timeout_sec = 0`
- **WHEN** вызывается любой query-инструмент
...
- `internal/mcp/registry.go` — registry всех MCP tools (query, RTI, TRC, review, health, stats), `toolHandler(ctx, args)` сигнатура
...
- `internal/config/config.go` — `MCPConfig.QueryTimeoutSec`, `MCPConfig.ReviewTimeoutSec`
