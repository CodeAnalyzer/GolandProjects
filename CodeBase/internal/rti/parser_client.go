...
// SQL-блоков в сводке, аналогично порогу для серверных вызовов.
// Вычисляется из rtiSlowThresholdMs (настраивается через SetSlowThresholdMs).
func clientSlowSQLThresholdSec() float64 {
	return float64(rtiSlowThresholdMs) / 1000.0
}
...
func FillClientSummary(s *RTISummary, events []*RTIClientEvent) {
	...
	s.TopSlowClientSQL = topSlowClientSQLEvents(events, rtiTopSlowCount)
}
